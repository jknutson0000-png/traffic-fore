...
export const TrafficDashboard: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 pb-20 animate-fade-in">
      
      {/* Header */}
      <header className="bg-black border-b border-red-900/30 py-4 px-4 md:px-6 flex justify-between items-center sticky top-0 z-50 shadow-2xl shadow-red-900/10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg md:text-2xl font-black tracking-tighter text-red-600 italic leading-none">
              TRAFFIC <span className="text-slate-100 not-italic">COMMAND</span>
            </h1>
            <p className="text-[10px] text-slate-500 font-mono uppercase tracking-widest hidden md:block">20 Marietta St NW</p>
          </div>
        </div>
        <button 
          onClick={() => fetchTraffic(viewMode)}
          disabled={loading}
          className="bg-red-900/20 hover:bg-red-900/40 text-red-500 hover:text-red-400 px-3 py-2 rounded font-black text-xs md:text-sm border border-red-900/50 transition-all disabled:opacity-50 flex items-center gap-2 uppercase tracking-wider"
        >
          {loading ? (
                       ...
                       <div key={idx} className={`p-6 border-2 rounded-sm relative overflow-hidden transition-all hover:scale-[1.01] ${
                          ...
                          <div className="flex justify-between items-start mb-4 relative z-10">
                             ...
                             <div className={`px-3 py-1 text-xs font-black uppercase tracking-widest border ${
                                day.level === 'RED' ? 'bg-red-600 text-white border-red-400' :
                                day.level === 'YELLOW' ? 'bg-yellow-600 text-black border-yellow-400' :
                          ...
                          <h4 className={`text-xl font-bold mb-2 relative z-10 ${
                             day.level === 'RED' ? 'text-red-400' : 
                             day.level === 'YELLOW' ? 'text-yellow-400' : 'text-emerald-400'
                          }`}>
                             "{day.headline}"
                          </h4>
